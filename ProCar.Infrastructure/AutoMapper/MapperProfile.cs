...
namespace ProCar.Infrastructure.AutoMapper
{
    public class MapperProfile : Profile
    {
        public MapperProfile()
        {
            ...
            CreateMap<Leases, LeaseViewModel>().ForMember(x => x.leasestatus, x => x.ToString()).ForMember(x => x.LegaldocumentImegUrl, x => x.Ignore()).ForMember(x => x.StartRent, x => x.MapFrom(x => x.StartRent.ToString("yyyy:MM:dd"))).ForMember(x => x.EndRent, x => x.MapFrom(x => x.EndRent.ToString("yyyy:MM:dd")));
